...
public class ImageAdjuster implements PConstants {
   ...
   */
  public final void apply(PImage imgDest, PImage imgSrc) {
    ...
    int [] pixDest = imgDest.pixels;
    int [] pixSrc = imgSrc.pixels;
    int area = imgSrc.width * imgSrc.height;
    int index = 0;
    // perform adjustment
    do {
      int c = pixSrc[index];
      pixDest[index] = (c & 0xFF000000) | (ilut[c>>16&0xFF] << 16) | (ilut[c>>8&0xFF] << 8) | (ilut[c&0xFF]);
    } while (++index < area);
    if (autoMaintainPixels)
   ...
   * @see #apply(PImage)
   */
  public final void brightness(PImage imgDest, PImage imgSrc, float amount) {
    //checkImage(img);
    reset();
    brightness(amount);
    apply(imgDest, imgSrc);
